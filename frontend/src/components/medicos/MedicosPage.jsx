...
import AgendarAgendamentoModal from '../agendamentos/AgendarAgendamentoModal';
...
function MedicosPage() {
  ...
  const [medicoSelecionadoAgendamento, setMedicoSelecionadoAgendamento] =
    useState(null);
  ...
  async function handleDeletar() {
    ...
    try {
      ...
      const mensagem =
        ...
        'Não foi possível excluir este médico. Verifique se existe algum agendamento vinculado antes de tentar novamente.';
  ...
  async function handleExcluirSelecionados() {
    ...
    try {
      ...
      const medicosSelecionados = medicos.filter((medico) =>
        selecionados.includes(medico.id)
      );

      const resultados = await Promise.allSettled(
        medicosSelecionados.map(async (medico) => {
          try {
            ...
            return {
              ...
              mensagem:
                error?.response?.data?.mensagem ||
                'Não foi possível excluir este médico. Verifique se existe algum agendamento vinculado.',
            };
          }
        })
      );
      ...
      const sucessos = respostas.filter((item) => item.sucesso);
      const falhas = respostas.filter((item) => !item.sucesso);
      ...
      if (sucessos.length > 0 && falhas.length === 0) {
        ...
        toast.success(
          `${sucessos.length} ${
            sucessos.length === 1 ? 'médico excluído' : 'médicos excluídos'
          }. ${falhas.length} ${
            falhas.length === 1
              ? 'não pôde ser excluído por possuir vínculo.'
              : 'não puderam ser excluídos por possuírem vínculos.'
          }`
        );
  ...
  async function handleSalvarAgendamento(agendamento) {
    try {
      ...
      const mensagem =
        error?.response?.data?.mensagem || 'Erro ao realizar agendamento.';
  ...
  return (
    <div className="h-[calc(100vh-44px)] overflow-hidden bg-gray-100 p-4">
      ...
      <ConfirmDialog
        ...
        }? Médicos com agendamentos vinculados não poderão ser excluídos.`}
        confirmText="Excluir"
        loading={bulkDeleting}
      />

      <AgendarAgendamentoModal
