...
function RelatorioMinutosPage() {
  ...
  const agendamentosFiltrados = useMemo(() => {
    ...
    return agendamentos
      .filter(
        (agendamento) =>
          agendamento.dataInicioAtendimento && agendamento.dataFimAtendimento
      )
      .filter((agendamento) => {
        const dataAgendamento = formatarDataInput(agendamento.dataAgendamento);
        ...
        const bateDataInicio = dataInicio ? dataAgendamento >= dataInicio : true;
        const bateDataFim = dataFim ? dataAgendamento <= dataFim : true;
  ...
  }, [agendamentos, busca, dataInicio, dataFim, statusFiltro]);

  const resumo = useMemo(() => {
    const totalMinutos = agendamentosFiltrados.reduce((total, agendamento) => {
      return (
        total +
        calcularMinutos(
    ...
    const media = totalAgendamentos > 0 ? totalMinutos / totalAgendamentos : 0;
    ...
    <div className="h-[calc(100vh-44px)] overflow-hidden bg-gray-100 p-4">
      <div className="grid h-full grid-rows-[auto_auto_auto_1fr] gap-3">
        ...
        <div className="grid grid-cols-4 gap-3">
          <ResumoCard
            titulo="Agendamentos"
            valor={resumo.totalAgendamentos}
            descricao="Com atendimento"
            icon={CalendarDays}
            cor="blue"
          />
          <ResumoCard
            titulo="Minutos"
            valor={Math.round(resumo.totalMinutos)}
          ...
          <ResumoCard
            titulo="Horas"
            valor={resumo.horas.toFixed(1)}
            descricao="Equivalente total"
            icon={Activity}
            cor="violet"
          />
          <ResumoCard
            titulo="Média"
            valor={`${Math.round(resumo.media)} min`}
            descricao="Por agendamento"
            icon={Timer}
            cor="amber"
          />
        </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Buscar por ID, paciente, médico ou procedimento..."
              ...
              agendamentosFiltrados.map((agendamento) => (
                ...
                >
                  ...
                  <span className="flex items-center gap-2 truncate font-medium text-gray-800">
                    ...
                    {agendamento.paciente?.nome || 'Não informado'}
                  </span>
                  ...
                  <span>{formatarHora(agendamento.dataInicioAtendimento)}</span>
                  <span>{formatarHora(agendamento.dataFimAtendimento)}</span>
