  ...
  const pagamentosFiltrados = useMemo(() => {
    ...
    return pagamentos.filter((pagamento) => {
      const dataPagamento = formatarDataInput(
        pagamento.dataPagamento || pagamento.dataCadastro || pagamento.agendamento?.dataAgendamento
      );

      const bateBusca =
        !termo ||
        String(pagamento.id || '').includes(termo) ||
        String(pagamento.agendamentoId || '').includes(termo) ||
        (pagamento.agendamento?.paciente?.nome || '').toLowerCase().includes(termo) ||
        (pagamento.agendamento?.medico?.nome || '').toLowerCase().includes(termo);
