...
function ProntuariosPage() {
  ...
  const prontuariosFiltrados = useMemo(() => {
    ...
    return prontuarios.filter((prontuario) => {
      return (
        ...
        (prontuario.agendamento?.paciente?.nome || '').toLowerCase().includes(termo) ||
        (prontuario.agendamento?.medico?.nome || '').toLowerCase().includes(termo) ||
        (prontuario.agendamento?.procedimento?.nome || '').toLowerCase().includes(termo)
      );
    });
  ...
  const totalAgendamentosDisponiveis = agendamentos.length;
        ...
        <div className="rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar por ID, paciente, médico, diagnóstico, receita, observação ou agendamento..."
                value={searchTerm}
            ...
            <div className="flex h-9 items-center rounded-lg border border-gray-200 bg-gray-50 px-3 text-xs text-gray-600">
              ...
              {totalAgendamentosDisponiveis > 0 && (
                <span className="ml-3 inline-flex items-center gap-1 rounded-full bg-violet-100 px-2 py-1 text-[11px] font-medium text-violet-700">
                  ...
                  {totalAgendamentosDisponiveis} agendamentos
                </span>
              )}
